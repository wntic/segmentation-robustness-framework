...
from segmentation_robustness_framework.attacks.rfgsm import RFGSM
from tests.attacks.base_attack_common import model  # noqa: F401


@pytest.fixture
def attack(model):  # noqa: F811
    return RFGSM(model, eps=0.1, alpha=0.01, iters=5)


def test_initialization(model):  # noqa: F811
    attack = RFGSM(model, eps=0.1, alpha=0.01, iters=5, targeted=False)
...
def test_apply_with_different_eps_values(model):  # noqa: F811
    ...
    for eps in eps_values:
        attack = RFGSM(model, eps=eps, alpha=0.01, iters=3)
        model.eval()
...
def test_apply_with_different_alpha_values(model):  # noqa: F811
    ...
    for alpha in alpha_values:
        attack = RFGSM(model, eps=0.1, alpha=alpha, iters=3)
        model.eval()
...
def test_apply_with_different_iterations(model):  # noqa: F811
    ...
    for iters in iterations:
        attack = RFGSM(model, eps=0.1, alpha=0.01, iters=iters)
        model.eval()
...
def test_apply_targeted_vs_untargeted(model):  # noqa: F811
    ...
    attack_untargeted = RFGSM(model, eps=0.1, alpha=0.01, iters=3, targeted=False)
    attack_targeted = RFGSM(model, eps=0.1, alpha=0.01, iters=3, targeted=True)
