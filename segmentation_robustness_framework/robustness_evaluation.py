...
class RobustnessEvaluation:
    ...
    Attributes:
        ...
        save_dir (Path): Directory path where the evaluation results and images will be saved.
    """
    ...
    def run(self, show: bool = False) -> None:
        ...
        for group_idx, attack_group in enumerate(attacks_list):
            ...
            self._evaluate_attacks(model, dataset, num_images, group_idx, attack_group, show)
    ...
    def _prepare_save_dir(self) -> None:
        ...
        self.run_dir = os.path.join(self.save_dir, f"run_{len(os.listdir(self.save_dir))}")
    ...
    def _evaluate_attacks(
        self, model: nn.Module, dataset: Dataset, num_images: int, group_idx: int, attack_group: int, show: bool
    ) -> None:
